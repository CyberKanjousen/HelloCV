...
是的，程序在运行到这里时，会停顿30毫秒，以保证帧的更迭不要过快。但是，在这30毫秒中，程序什么也不干，只是等待30毫秒倒计时结束。

这真是太\*问候语*弱智了。于是，我决定异步处理图像。出于~~懒~~尽量减少修改代码的需要，我用lambda表达式将处理图像的部分包含起来，再把这个lambda表达式放进新建的线程里，最后稍微修改一下代码顺序，优化就完成了：
