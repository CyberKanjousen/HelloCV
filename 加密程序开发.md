...
> <font style="color:rgb(51, 51, 51);background-color:rgb(250, 252, 253);">`long int strtol(const char *str, char **endptr, int base)`把参数`str`所指向的字符串根据给定的`base`转换为一个长整数（类型为`long int`型），`base`必须介于`2`和`36`（包含）之间，或者是特殊值`0`。</font>
>
> + <font style="color:rgb(51, 51, 51);background-color:rgb(250, 252, 253);">`str` -- 要转换为长整数的字符串。</font>
> + <font style="color:rgb(51, 51, 51);background-color:rgb(250, 252, 253);">`endptr` -- 对类型为`char*`的对象的引用，其值由函数设置为`str`中数值后的下一个字符。</font>
> + <font style="color:rgb(51, 51, 51);background-color:rgb(250, 252, 253);">`base` -- 基数，必须介于`2`和`36`（包含）之间，或者是特殊值`0`。如果`base`为 0，则会根据字符串的前缀来判断进制：如果字符串以`0x`或`0X`开头，则将其视为十六进制；如果字符串以`0`开头，则将其视为八进制；否则将其视为十进制。</font>
>
